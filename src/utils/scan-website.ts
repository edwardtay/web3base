...
export async function scanWebsite(url: string): Promise<string> {
  try {
    ...
    // Check if this is an ENS domain - should not be scanned as URL
    const ensPattern = /^[a-zA-Z0-9-]+\.eth$/i;
    if (ensPattern.test(url.trim())) {
      logger.warn(`[A2A] Rejecting ENS domain ${url} - not a website URL`);
      return JSON.stringify({
        error: "ens_domain_not_url",
        raw: url,
        message: `"${url}" is an ENS domain name, not a website URL. ENS domains should be resolved to wallet addresses for security audits, not scanned as websites.`,
        suggestion: "Please use the wallet audit feature instead, which will resolve the ENS domain to its wallet address.",
      }, null, 2);
    }
    ...
    // Double-check: if domain ends with .eth after parsing, reject it
    if (parsed.hostname.toLowerCase().endsWith('.eth')) {
      logger.warn(`[A2A] Rejecting ENS domain ${parsed.hostname} - not a website URL`);
      return JSON.stringify({
        error: "ens_domain_not_url",
        raw: url,
        message: `"${parsed.hostname}" is an ENS domain name, not a website URL. ENS domains should be resolved to wallet addresses for security audits.`,
        suggestion: "Please use the wallet audit feature instead.",
      }, null, 2);
    }
