...
export class UnifiedActionProvider extends ActionProvider<WalletProvider> {
  ...
  /**
   * Circle - Get USDC balance
   */
  @CreateAction({
    name: "circle_get_balance",
    description:
      "Get USDC balance for a wallet address using Circle API. Requires CIRCLE_API_KEY.",
    schema: z.object({
      address: z.string().describe("Wallet address to check USDC balance"),
    }),
  })
  async circleGetBalance(
    walletProvider: WalletProvider,
    args: { address: string },
  ): Promise<string> {
    try {
      logger.info("Circle: Getting USDC balance", args);
      const client = getCircleClient();
      const balance = await client.getUSDCBalance(args.address);
      ...
      logger.error("Circle balance error", error);
      return `Error getting Circle balance: ${error instanceof Error ? error.message : String(error)}`;
    }
  }

  /**
   * Circle - Verify address security
   */
  @CreateAction({
    name: "circle_verify_address",
    description:
      "Verify wallet address security using Circle's verification service. Returns risk level.",
    schema: z.object({
      address: z.string().describe("Wallet address to verify"),
    }),
  })
  async circleVerifyAddress(
    walletProvider: WalletProvider,
    args: { address: string },
  ): Promise<string> {
    try {
      logger.info("Circle: Verifying address", args);
      const client = getCircleClient();
      const verification = await client.verifyAddress(args.address);
      ...
      logger.error("Circle verification error", error);
      return `Error verifying address: ${error instanceof Error ? error.message : String(error)}`;
    }
  }

  /**
   * ZetaChain - Get cross-chain transaction
   */
  @CreateAction({
    name: "zetachain_get_cross_chain_tx",
    description:
      "Get cross-chain transaction details from ZetaChain universal blockchain.",
    schema: z.object({
      txHash: z.string().describe("Transaction hash to query"),
    }),
  })
  async zetachainGetCrossChainTx(
    walletProvider: WalletProvider,
    args: { txHash: string },
  ): Promise<string> {
    try {
      logger.info("ZetaChain: Getting cross-chain transaction", args);
      const client = getZetaChainClient();
      const tx = await client.getCrossChainTx(args.txHash);
      ...
      logger.error("ZetaChain transaction error", error);
      return `Error getting ZetaChain transaction: ${error instanceof Error ? error.message : String(error)}`;
    }
  }

  /**
   * ZetaChain - Verify cross-chain message
   */
  @CreateAction({
    name: "zetachain_verify_message",
    description:
      "Verify cross-chain message integrity using ZetaChain.",
    schema: z.object({
      messageId: z.string().describe("Cross-chain message ID to verify"),
    }),
  })
  async zetachainVerifyMessage(
    walletProvider: WalletProvider,
    args: { messageId: string },
  ): Promise<string> {
    try {
      logger.info("ZetaChain: Verifying cross-chain message", args);
      const client = getZetaChainClient();
      const verification = await client.verifyCrossChainMessage(args.messageId);
      ...
      return `Error verifying message: ${error instanceof Error ? error.message : String(error)}`;
    }
  }

  /**
   * Seedify - Verify project security
   */
  @CreateAction({
    name: "seedify_verify_project",
    description:
      "Verify Web3 project security and audit status using Seedify launchpad.",
    schema: z.object({
      projectId: z.string().describe("Project ID to verify"),
    }),
  })
  async seedifyVerifyProject(
    walletProvider: WalletProvider,
    args: { projectId: string },
  ): Promise<string> {
    try {
      logger.info("Seedify: Verifying project", args);
      const client = getSeedifyClient();
      const verification = await client.verifyProject(args.projectId);
      ...
      return `Error verifying project: ${error instanceof Error ? error.message : String(error)}`;
    }
  }

  /**
   * Somnia - Get blockchain metrics
   */
  @CreateAction({
    name: "somnia_get_metrics",
    description:
      "Get high-performance blockchain metrics from Somnia infrastructure.",
    schema: z.object({}),
  })
  async somniaGetMetrics(
    walletProvider: WalletProvider,
    args: {},
  ): Promise<string> {
    try {
      logger.info("Somnia: Getting blockchain metrics");
      const client = getSomniaClient();
      const metrics = await client.getBlockchainMetrics();
      ...
      return `Error getting Somnia metrics: ${error instanceof Error ? error.message : String(error)}`;
    }
  }

  /**
   * NodeOps - Get node health
   */
  @CreateAction({
    name: "nodeops_get_node_health",
    description:
      "Get node infrastructure health status from NodeOps.",
    schema: z.object({
      nodeId: z.string().describe("Node ID to check health"),
    }),
  })
  async nodeopsGetNodeHealth(
    walletProvider: WalletProvider,
    args: { nodeId: string },
  ): Promise<string> {
    try {
      logger.info("NodeOps: Getting node health", args);
      const client = getNodeOpsClient();
      const health = await client.getNodeHealth(args.nodeId);
      ...
      return `Error getting node health: ${error instanceof Error ? error.message : String(error)}`;
    }
  }
}
